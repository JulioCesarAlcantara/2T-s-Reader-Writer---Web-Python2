...
#	print("aqui 4")
  ...
  def MFRC522_GetDataSectorTag(self, blockAddr):
    value = ""
    
    i = 0
    while(i < len(self.BackData)):
      if (i < 2) or (self.BackData[i] != 0):
        if(blockAddr == 1) or (blockAddr == 9):
          value = value + str(self.BackData[i])
        elif(blockAddr == 2) or (blockAddr == 8):
          value = value + chr(self.BackData[i])
      i = i + 1
